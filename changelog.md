...
- **验证状态**：静态检查通过写 `已验证（静态）`；在目标系统执行过写 `已实测`；
  尚未执行写 `待实测`；结论或实现被修订写 `已返工`，并说明返工后的实际状态。
...
- **返工（2026-08-09）**：本条把「验证状态」的取值写成 `待验证 / 已确认 / 有疑问`，
  与文档开头的现行口径（`已验证 / 待验证 / 已返工`）不一致，**以开头为准**。
- **验证状态**：待验证（原判「已返工」，已按上条返工）
...
2. **运营类（可用性 / 易用性 / 排障体验）问题中，触发几率不大的可以不处理。**
   ...
   用于判断不同场景是否受影响。
3. **本文档里的「待验证」暂时不管**：等安全性问题全部解决后再上真机实测，
...
- **本条不覆盖的**：`changelog.md` 里所有「待验证」按需求约束**暂不处理**，
...
- **验证状态**：已实测（退出码传播）；`bind-address` 待下轮完整安装确认
...
- **验证状态**：**pkg 路径已完整实测**（安装 → 建站 → 运维命令 → 升级）。
  **source 路径（源码编译）仅验证了验签环节**
  ...
  GOODSIG + VALIDSIG），**尚未执行完整的源码编译安装**：
...
**证书未真实签发**：验证机在 NAT 后面，且测试用的邮箱域名被 Let's Encrypt
...
- **验证状态**：
  ...
  - **Debian 13：未实测，但不构成阻断。** 用户确认**老版本 v2.1 已能在
...
- **注意：已经升级过的机器需要手工轮换**：密钥可预测这件事不会因为以后再升级而消除，
  ...
  后重新登录即可
- **验证状态**：已改，**未实测**（本阶段走的是首装路径，未执行 upgrade.sh phpmyadmin）
...
- **验证状态**：待实测（需登录 phpMyAdmin 验证 TempDir 生效、无 PHP 警告）
...
- **验证状态**：未实测：验证机装的是 LNMP，机上没有 Apache。
  LAMP / LNMPA 需要另起一台验证。
...
- **效果**：终端仍看得到密码去向（随机的给出 0600 文件路径），日志里只有
  "Install ... completed"。
- **验证状态**：已改，待执行 `./install.sh db` 确认日志中无密码
...
**从未执行过一次完整的 `./install.sh`。**

所有编译相关的判断仍然只是静态推理：configure 参数对不对、模块能不能编译进去、
扩展能不能加载，都没有被执行验证过。风险最高的几处按可能性排序：
...
> 下表是 2026-08-08 之前的状态，**除最后一行外均已完成**，保留以便追溯。
...
| **真实安装** | 尚未完整执行 `./install.sh` | 未通过：尚未完成 |
