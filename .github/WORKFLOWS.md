...
**每月 8 日会自动产生一个 Release**（有 AUTO 类更新时）。整条链跑两次 full 编译：
一次针对升级分支（这里的 Build Test），一次针对主分支（发布工作流内部），
因此耗时较长。没有 AUTO 类更新时 `push` 与 `release` 都跳过，不会产生空 Release。
